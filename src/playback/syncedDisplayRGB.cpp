#include "appinfo.h"
#include "syncedDisplayRGB.h"
#include "customExceptions.h"
#include "openglVariables.h"


syncedDisplayRGB::syncedDisplayRGB(QWidget *parent)
...
syncedDisplayRGB::~syncedDisplayRGB() {
  // to get current texture for deletion.
  makeCurrent();
}

void syncedDisplayRGB::setEngine(Mk03::engineContainer<>* ptr) {
  assert(ptr);
  pePtr = ptr;
  ...
  if(playFlag) {
      ...
      if (elapsedTime+updateTick > fpsInterval) {
          ...
          if(buf) {
              ...
              printBuf = buf;
              glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, mVideoWidth, mVideoHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, printBuf->buffer.data());
...
void syncedDisplayRGB::initializeGL() {
...
void syncedDisplayRGB::resizeGL(int a, int b) {
  Q_UNUSED(a);
  Q_UNUSED(b);

}

void syncedDisplayRGB::setPlayFlag(bool flag) {
  playFlag = flag;
  lastUpdate = std::chrono::steady_clock::now();
}

void syncedDisplayRGB::slotAudioPtsXBase_ms(int64_t ptsXBase) {
