#ifndef MK02_ENGINECONTAINER_H
#define MK02_ENGINECONTAINER_H

#include "appinfo.h"
#include "ffmpegUPtr.h"
#include "mySpsc/boost/lockfree/spsc_queue.hpp"
#include "Mk02/engineUtil.h"
#include "Mk02/playerDemuxer.h"
#include "Mk02/playerAudioDec.h"
#include "Mk02/playerVideoDec.h"
#include <cstddef>
#include <deque>
#include <mutex>
#include <thread>
...
public:
  engineContainer(std::string mediaSourceString,
                  AVPixelFormat aPFmt,
                  AVSampleFormat aSFmt,
                  int useAudioIndex = -42,
                  int useVideoIndex = -42,
                  int64_t seek_pts = -1)
  : jointData()
  ...
  void init(int useAudioIndex = -42, int useVideoIndex = -42, int64_t seek_pts = -1) {
    ...
    if (useAudioIndex == -42) {
        ...
        if (fc->streams[useAudioIndex]->codecpar->codec_type!=AVMEDIA_TYPE_AUDIO) throw std::invalid_argument("streamNotAudio");
    ...
    if (useVideoIndex == -42) {
        ...
        if (fc->streams[useVideoIndex]->codecpar->codec_type!=AVMEDIA_TYPE_VIDEO) throw std::invalid_argument("streamNotVideo");
  ...
  void startThreads() {
    threads.emplace_back(&Mk02::playerDemuxer::demux, mPlayerDemuxerPtr.get());
    threads.emplace_back(&Mk02::playerVideoDec::decodeVideo, mPlayerVideoDecPtr.get());
    if(mPlayerAudioDecPtr)
      threads.emplace_back(&Mk02::playerAudioDec::decodeAudio, mPlayerAudioDecPtr.get());
  }
...
public:
  ...
  videoBuffer *getVideoBuffer() {
    return popFromQueue(mPlayerVideoDecPtr->videoQueueSend);
  }
  ...
  int getSampleRate() const { if(mPlayerAudioDecPtr) return mPlayerAudioDecPtr->audioCodecContext->sample_rate; return 0; }
  int getChannelCount() const { if(mPlayerAudioDecPtr) return mPlayerAudioDecPtr->audioCodecContext->channels; return 0; }
  int getSampleBytes() const { if(mPlayerAudioDecPtr) return mPlayerAudioDecPtr->sampleBytes; return 0; }
  int getSampleBits() const { if(mPlayerAudioDecPtr) return mPlayerAudioDecPtr->sampleBytes*jointData.bitsPerByte; return 0; }
  ...
  int checkStreamType(int streamIndex) const {
    ...
    else return AVMEDIA_TYPE_UNKNOWN;
  }

  void seek(int64_t ptsXtimeBase) {
    int64_t seekTargetVideo = (ptsXtimeBase * mPlayerVideoDecPtr->timeBase.den) / (mPlayerVideoDecPtr->timeBase.num * 1000);
