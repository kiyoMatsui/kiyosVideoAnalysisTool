#ifndef MK02_PLAYERAUDIODEC_H
#define MK02_PLAYERAUDIODEC_H
...
public:
  playerAudioDec(engineJointData& ejd, uPtrAVFormatContext& afc, AVSampleFormat sFmt, bool flushFlag)
    ...
    for (unsigned int n = 0; n < jointData.audioBufferQueue ; ++n)
      audioBuffers.push_back({std::vector<std::byte>(
                               static_cast<unsigned int>(
                                 jointData.maxAudioFrameSampleNo
                                 *audioCodecContext->channels
                                 *sampleBytes)), 0, 0});
    for( auto& buffer : audioBuffers)
      pushToQueue(&buffer, audioQueueReturn);
  }
  ...
  void decodeAudio() {
    try {
      ...
      while (1) {
        ...
        if (!pkt) threadWait(jointData.packetAudioQueueSendMutex, jointData.packetAudioQueueSendCV, jointData.packetAudioQueueSend, jointData.endFlag);
        if(jointData.endFlag.load()) break;
